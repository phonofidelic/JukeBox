...
const library_reducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case POST_TRACK_DATA_SUCCESS:
      ...
      const updateIndex = state.tracks.findIndex(
        track => track._id === action.updatedTrack._id
      );
    ...
    case DELETE_TRACK_SUCCESS:
      const deleteIndex = state.tracks.findIndex(
        track => track._id === action.deletedTrack._id
      );
    ...
    case ORDER_TRACKS_BY_FIELD_VALUE:
      ...
      const BY_ARTIST = _.sortBy(
        _.sortBy(_.sortBy(tracksToSort, ['order', 'no']), ['album', 'title']),
        ['artist', 'name']
      );
      const BY_ALBUM = _.sortBy(_.sortBy(tracksToSort, ['order', 'no']), [
        'album',
        'title'
      ]);
    ...
    case FETCH_DETAIL_VIEW_SUCCESS:
      return {
        ...
        detailViewData: {
          ...
          imgSrc: action.detailViewData.artwork
            ? action.detailViewData.artwork[0].src
            : 'defaultImage'
        }
      };
    ...
    case TOGGLE_SEARCH:
      return {
        ...
        searchResults:
          state.searchResults.length > 0 ? state.searchResults : state.tracks
      };
    ...
    case SEARCH_LIBRARY:
      ...
      return {
        ...state,
        searchResults: state.tracks.filter(track => {
          if (
            (track.title.toLowerCase().includes(term) > 0) |
            (track.album.title.toLowerCase().includes(term) > 0) |
            (track.artist.name.toLowerCase().includes(term) > 0)
          ) {
            return track;
          }
        })
