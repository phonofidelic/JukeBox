...
class LibraryDesktop extends Component {
  ...
  sortByField(list, order, orderBy) {
    ...
    if (orderBy === 'artist') {
      return order === 'desc'
        ...
        : _.sortBy(_.sortBy(list, ['order', 'no']), [
            'artist',
            'name'
          ]).reverse();
    }
  ...
  render() {
    const { library, order, orderBy, handleRequestSort } = this.props;
    ...
    const styles = {
      ...
      tableContainer: {
        ...
        margin: `${theme.dimensions.navDesktop.marginTop}px auto ${theme
          .dimensions.navDesktop.marginTop * 2}px auto`,
        overflowX: 'auto'
      }
    };
